...
5. GKE GCloud Auth Plugin: When deploying this module (whether locally or via CI), you will need to have the `gcloud` CLI installed and additionally the GKE auth plugin. This can be done by running the command `gcloud components install gke-gcloud-auth-plugin`.
...
| <a name="input_cloudaccess_resource_suffix"></a> [cloudaccess\_resource\_suffix](#input\_cloudaccess\_resource\_suffix) | The resource suffix to use for CloudAccess resources. | `string` | `""` | no |
...
| <a name="input_gke_version"></a> [gke\_version](#input\_gke\_version) | The version to use for GKE. | `string` | `"1.30"` | no |
